...
def plot(data, cel_visible=True, bounce_visible=False):
    ...
    for col in filter(r.match, data.columns):
        ...
        color = {'confirmed':'blue', 'deaths': 'red', 'recovered': 'green'}[set]
        # plot data
        g = data.groupby([data.index, 'phase'])
        g[f'data_{set}'].sum().unstack().plot(ax=ax_data, linestyle='None',
                                              marker='x',
                                              # color='red',
                                              color=color
                                              )
        ...
        # plot celeration
        if cel_visible:
            cel.plot(y=f'celeration_{set}', ax=ax_data,
                     # color='black',
                     color=color,
                     alpha=0.75)
        # plot bounce
        if bounce_visible:
            cel.plot(y=[f'up_bounce_{set}', f'down_bounce_{set}'], ax=ax_data,
                     # color='black',
                     color=color,
                     alpha=0.5)
